...
function calcolaAnticipoPartner() {
  ...
  document.getElementById('anticipoImportoPartner').textContent = '-' + formatEuro(anticipoPartner);
  document.getElementById('anticipoSpesaEffettiva').textContent = formatEuro(spesaEffettiva);
}
...
function salva(e) {
  ...
  if (anticipoPartner > 0) {
    ...
    console.log('[ANTICIPO PARTNER] Hai anticipato formatEuro(anticipoPartner) + ' per il partner');
    console.log('[ANTICIPO PARTNER] Saldo condiviso aggiornato: formatEuro(DB.splitwise.saldo));
    console.log('[ANTICIPO PARTNER] Il partner ti deve formatEuro(anticipoPartner) + ' IN PIÙ (o tu devi IN MENO)');
  }
      ...
      header.innerHTML =
        ...
        '<div class="trans-content">' +
          '<div class="trans-main-row">' +
            ...
            '<div class="trans-amount">' + amountPrefix + formatEuro(t.importo) + '</div>' +
          '</div>' +
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    // Box stats - flex garantisce altezze identiche
    html += '<div style="display:flex;gap:8px;margin-bottom:14px">';

    html += '<div style="flex:1;padding:10px 4px;background:#27ae60;border-radius:10px;text-align:center;display:flex;flex-direction:column;justify-content:center;align-items:center">';
    html += '<div style="font-size:0.6em;color:rgba(255,255,255,0.85);font-weight:700;text-transform:uppercase;white-space:nowrap;margin-bottom:5px">💵 Risparmiato</div>';
    html += '<div style="font-size:0.92em;font-weight:800;color:#fff">' + formatEuro(risparmiato) + '</div>';
    html += '</div>';

    html += '<div style="flex:1;padding:10px 4px;background:#2196f3;border-radius:10px;text-align:center;display:flex;flex-direction:column;justify-content:center;align-items:center">';
    html += '<div style="font-size:0.6em;color:rgba(255,255,255,0.85);font-weight:700;text-transform:uppercase;white-space:nowrap;margin-bottom:5px">🎯 Obiettivo</div>';
    html += '<div style="font-size:0.92em;font-weight:800;color:#fff">' + formatEuro(obj.target) + '</div>';
    html += '</div>';

    html += '<div style="flex:1;padding:10px 4px;background:#ff9800;border-radius:10px;text-align:center;display:flex;flex-direction:column;justify-content:center;align-items:center">';
    html += '<div style="font-size:0.6em;color:rgba(255,255,255,0.85);font-weight:700;text-transform:uppercase;white-space:nowrap;margin-bottom:5px">💰 Mancante</div>';
    html += '<div style="font-size:0.92em;font-weight:800;color:#fff">' + formatEuro(mancante) + '</div>';
    html += '</div>';
...
function calcolaStoriaPatrimonio() {
  ...
  transazioniOrdinate.forEach(function(t, index) {
    ...
    patrimonioHistory.push({
      ...
      transazione: t,
      descrizione: (t.tipo === 'income' ? '+' : '-') + 'formatEuro(importo) + ' ' + (t.categoria || '') + ' ' + (t.note || '')
    });
  });
