...
function eseguiImport(modalita) {
  ...
  if (modalita === 'preserva') {
    ...
    if (contiBackup && contiBackup.buoni_pasto) {
      if (!DB.conti) DB.conti = {};
      DB.conti.buoni_pasto = contiBackup.buoni_pasto;
    }
...
function aggiornaWidgetObiettivi() {
  ...
  DB.obiettivi.slice(0, 3).forEach(function(obj) {
    // NUOVO: Calcola risparmio automatico dalle transazioni nel periodo
    var risparmiato = obj.risparmiato || 0;
    if (obj.dataInizio) {
      ...
      DB.transazioni.forEach(function(t) {
        ...
        if (d >= dataInizio && d <= oggi) {
          var imp = parseFloat(t.importo) || 0;
          if (t.tipo === 'income') risparmioCalcolato += imp;
          else if (t.tipo === 'expense') risparmioCalcolato -= imp;
        }
      });

      if (risparmioCalcolato > 0) {
        risparmiato = risparmioCalcolato;
      }
    }
    ...
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;gap:10px">';
    html += '<div style="font-weight:700;font-size:0.95em;color:#2c3e50;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + obj.icona + ' ' + obj.nome + '</div>';
    html += '<div style="font-size:0.85em;font-weight:800;color:' + progressColor + ';flex-shrink:0">' + percentuale.toFixed(0) + '%</div>';
    html += '</div>';
    ...
    html += '<span>' + formatEuro(risparmiato) + '</span>';
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    if (obj.dataInizio) {
      ...
      // Usa il valore calcolato se è positivo, altrimenti mantieni il manuale
      if (risparmioCalcolato > 0) {
        risparmiato = risparmioCalcolato;
      }
    }
  ...
  // Aggiorna totale risparmiato (con calcolo automatico)
  var totaleRisparmiato = DB.obiettivi.reduce(function(sum, obj) {
    ...
    if (obj.dataInizio) {
      ...
      if (risparmioCalcolato > 0) risparmiato = risparmioCalcolato;
    }
    return sum + risparmiato;
  }, 0);
