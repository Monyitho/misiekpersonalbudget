...
function aggiorna() {
  ...
  if (document.getElementById('daRecuperareMese')) {
    ...
    if (daRecuperareMese > 0.01) {
      ...
      labelMese.textContent = 'Partner Ti Deve (Questo Mese)';
      ...
      labelMese.textContent = 'Tu Devi al Partner (Questo Mese)';
...
function aggiornaAnalisiRisparmio(entrate, speseNecessita, speseDesideri, risparmioSulConto, daRecuperare, risparmioTotale, budgetNecessita, budgetDesideri, budgetRisparmi, totaleUscite) {
  ...
  // ── Card 1: Risparmio del Mese (flat, un solo livello) ──
  html += '<div class="card" style="margin-bottom:10px">';
  html += '<div class="card-section-title"><span class="cst-emoji">💰</span><span class="cst-text">Risparmio del Mese</span></div>';

  // Sul Conto
  html += '<div class="stat-flat-row c-blue">';
  html += '<div><div class="stat-flat-label">Sul Conto</div><div class="stat-flat-sub">Cash flow reale</div></div>';
  html += '<div class="stat-flat-val c-blue">' + formatEuro(risparmioSulConto) + '</div>';
  html += '</div>';

  // Da recuperare / rimborsare / pari
  var daRecupClass = 'c-orange';
  var daRecupLabel = 'Pari';
  var daRecupSub = 'Splitwise';
  if (daRecuperare > 0.01)  { daRecupClass = 'c-green'; daRecupLabel = 'Da Recuperare'; }
  else if (daRecuperare < -0.01) { daRecupClass = 'c-red'; daRecupLabel = 'Da Rimborsare'; }
  html += '<div class="stat-flat-row ' + daRecupClass + '">';
  html += '<div><div class="stat-flat-label">' + daRecupLabel + '</div><div class="stat-flat-sub">' + daRecupSub + '</div></div>';
  html += '<div class="stat-flat-val ' + daRecupClass + '">' + (daRecuperare >= 0 ? '+' : '') + formatEuro(Math.abs(daRecuperare)) + '</div>';
  html += '</div>';

  // Separatore
  html += '<div style="border-top:1px dashed rgba(255,255,255,0.1);margin:10px 0"></div>';

  // Totale strip
  html += '<div class="total-strip">';
  html += '<div><div class="ts-label">Risparmio Totale</div><div class="ts-sub">Conto + Splitwise</div></div>';
  html += '<div class="ts-val">' + formatEuro(risparmioTotale) + '</div>';
  html += '</div>';

  // KPI: obiettivo e raggiunto
  var percRagg = budgetRisparmi > 0 ? ((risparmioTotale / budgetRisparmi) * 100).toFixed(0) : '0';
  var raggClass = risparmioTotale >= budgetRisparmi ? 'c-green' : 'c-orange';
  html += '<div class="kpi-boxes-row">';
  html += '<div class="kpi-box-item c-blue"><div class="kpi-box-label">Obiettivo 15%</div><div class="kpi-box-val c-blue">' + formatEuro(budgetRisparmi) + '</div></div>';
  ...
  var bannerMsg   = differenza >= 0
    ...
    : 'Ti mancano ' + formatEuro(Math.abs(differenza)) + ' per raggiungere l\'obiettivo del 15%';
  ...
  html += '<div style="display:flex;justify-content:space-between;margin-bottom:5px;font-size:0.88em">';
  html += '<span>💸 <strong>Costo Reale</strong></span><span><strong>' + formatEuro(totaleUscite) + '</strong> · ' + percentualeUscite.toFixed(1) + '%</span>';
  html += '</div>';
  html += '<div style="background:rgba(255,255,255,0.1);height:8px;border-radius:4px;overflow:hidden">';
...
function aggiornaComposizioneConti() {
  ...
  if (conti.length === 0) {
    html = '<div style="text-align:center;color:rgba(255,255,255,0.5);font-size:0.85em;padding:10px 0">Nessun conto con saldo attivo</div>';
    container.innerHTML = html;
    return;
  }
...
function mostraCalendario() {
  ...
  labelEl.textContent = mesiNomi[calendarMonth] + ' ' + calendarYear;
  ...
  DB.transazioni.forEach(function(t) {
    ...
    var imp = parseFloat(t.importo) || 0;
    if (t.tipo === 'income') perGiorno[giorno].entrate += imp;
    else perGiorno[giorno].uscite += imp;
...
function apriFormRicorrente(idx) {
  ...
  var fsMonth = fs + ';overflow:hidden;max-width:100%';
  ...
  modal.innerHTML = '<div style="background:var(--card);border-radius:24px 24px 0 0;padding:24px 20px 40px;width:100%;max-width:480px;max-height:90vh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;box-sizing:border-box">'
    ...
    + '<div style="margin-bottom:14px;overflow:hidden;width:100%;box-sizing:border-box"><label style="' + ls + '">Mese/Anno di inizio</label>'
    + '<input type="month" id="ricDataInizio" value="' + defaultInizio + '" style="' + fsMonth + '"></div>'

    + '<div style="margin-bottom:24px;overflow:hidden;width:100%;box-sizing:border-box"><label style="' + ls + '">Mese/Anno di fine (lascia vuoto = indefinita)</label>'
    + '<input type="month" id="ricDataFine" value="' + defaultFine + '" style="' + fsMonth + '"></div>'
