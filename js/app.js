...
function aggiornaListaObiettiviModal() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    html += '<div style="background:var(--card);padding:12px;border-radius:12px;margin-bottom:10px;border-left:4px solid ' + progressColor + '">';

    // Header: icona + nome + pulsanti
    html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">';
    html += '<div style="font-size:1.5em;flex-shrink:0">' + obj.icona + '</div>';
    html += '<div style="flex:1;min-width:0;font-weight:700;font-size:0.95em;color:var(--text)">' + obj.nome + '</div>';
    html += '<button onclick="modificaRisparmiato(' + index + ')" style="background:#3498db;color:white;border:none;width:36px;height:36px;border-radius:8px;cursor:pointer;font-size:1em;flex-shrink:0">✏️</button>';
    html += '<button onclick="eliminaObiettivo(' + index + ')" style="background:#e74c3c;color:white;border:none;width:36px;height:36px;border-radius:8px;cursor:pointer;font-size:1em;flex-shrink:0">🗑️</button>';
    html += '</div>';

    // Target e risparmiato
    html += '<div style="text-align:center;margin-bottom:6px">';
    html += '<div style="font-size:0.8em;color:#7f8c8d">Target: <strong>' + formatEuro(obj.target) + '</strong></div>';
    html += '<div style="font-size:0.95em;font-weight:600;color:' + progressColor + '">' + formatEuro(risparmiato) + ' risparmiati</div>';
    html += '</div>';

    // Barra progresso con percentuale
    html += '<div style="margin-bottom:6px">';
    html += '<div style="background:var(--border);height:8px;border-radius:4px;overflow:hidden">';
    html += '<div style="background:' + progressColor + ';height:100%;width:' + Math.min(percentuale, 100) + '%;border-radius:4px"></div>';
    html += '</div>';
    html += '<div style="text-align:center;font-size:0.8em;font-weight:700;color:' + progressColor + ';margin-top:4px">' + percentuale + '%</div>';
    html += '</div>';

    // Date su una riga
    if (obj.dataInizio || obj.dataFine) {
      html += '<div style="display:flex;justify-content:center;gap:12px;font-size:0.7em;color:#888">';
      if (obj.dataInizio) {
        var dInizio = new Date(obj.dataInizio);
        html += '<span>Inizio: ' + dInizio.getDate() + ' ' + ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'][dInizio.getMonth()] + ' ' + dInizio.getFullYear() + '</span>';
      }
      if (obj.dataFine) {
        var dFine = new Date(obj.dataFine);
        html += '<span>Fine: ' + dFine.getDate() + ' ' + ['Gen','Feb','Mar','Apr','Mag','Giu','Lug','Ago','Set','Ott','Nov','Dic'][dFine.getMonth()] + ' ' + dFine.getFullYear() + '</span>';
      }
      html += '</div>';
    }
