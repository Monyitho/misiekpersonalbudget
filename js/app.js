...
function aggiorna() {
  ...
  // ========== CASCATA MENSILE ==========
  aggiornaCascataMensile(ent, usciteReali, saldo);
...
// ========== CASCATA MENSILE ==========
function aggiornaCascataMensile(entrate, usciteReali, cashConto) {
  ...
  var elCash    = document.getElementById('cascCash');
  ...
  var elIcon    = document.getElementById('cascRisparmioIcon');
  if (!elEntrate) return;

  elEntrate.textContent = formatEuro(entrate);
  elUscite.textContent  = '−' + formatEuro(usciteReali);
  elCash.textContent    = formatEuro(cashConto);
  elCash.style.color    = cashConto >= 0 ? '#e5e4e2' : '#fca5a5';

  // Splitwise row
  elSplitLbl.textContent = 'Saldo con ' + pName;
  if (splitwiseMese > 0) {
    elSplit.textContent = '+' + formatEuro(splitwiseMese);
    elSplit.style.color = '#4ecca3';
    elSplitSub.textContent = pName + ' ti deve questo mese';
  } else if (splitwiseMese < 0) {
    elSplit.textContent = '−' + formatEuro(Math.abs(splitwiseMese));
    elSplit.style.color = '#fca5a5';
    elSplitSub.textContent = 'devi ancora a ' + pName + ' questo mese';
  } else {
    elSplit.textContent = '±€0,00';
    elSplit.style.color = '#71717a';
    elSplitSub.textContent = 'pari questo mese';
  }

  // Risparmio reale
  elRisp.textContent   = (risparmioReale >= 0 ? '+' : '−') + formatEuro(Math.abs(risparmioReale));
  elRisp.style.color   = risparmioReale >= 0 ? '#4ecca3' : '#fca5a5';
  elIcon.textContent   = risparmioReale >= 0 ? '✅' : '🔴';
}
