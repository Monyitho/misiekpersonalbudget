...
function aggiorna() {
  ...
  DB.transazioni.forEach(function(t) {
    ...
    // NUOVO: Pago al Partner (saldo debito) - RIDUCE suo debito verso di me (o aumenta il mio verso di lei)
    if (t.saldoDebito && t.tipo === 'expense') {
      var imp = parseFloat(t.importo) || 0;
      // Se io pago il partner = riduco il mio credito / aumento il mio debito
      recuperiVirtualiLeiPaga += imp;
    }
  });
...
// ========== PAGO AL PARTNER (SALDO DEBITO) ==========
function togglePagoPartner() {
  var checkbox = document.getElementById('tPagoPartner');
  if (!checkbox) return;

  checkbox.checked = !checkbox.checked;
  aggiornaVisualCheckboxPagoPartner();

  // Mostra/nascondi info box
  var infoBox = document.getElementById('pagoPartnerInfo');
  if (infoBox) {
    infoBox.style.display = checkbox.checked ? 'block' : 'none';
  }

  // Se attivato, disattiva "Spesa Condivisa" (non ha senso insieme)
  if (checkbox.checked) {
    var condivisoCheckbox = document.getElementById('tcondiviso');
    if (condivisoCheckbox && condivisoCheckbox.checked) {
      condivisoCheckbox.checked = false;
      aggiornaVisualCheckbox();
    }
  }
...
function aggiornaVisualCheckboxPagoPartner() {
  var checkbox = document.getElementById('tPagoPartner');
  var checkIcon = document.getElementById('pagoPartnerCheckIcon');
  var customCheckbox = document.getElementById('pagoPartnerCheckbox');
  var toggle = document.getElementById('pagoPartnerToggle');

  if (!checkbox || !checkIcon || !customCheckbox) return;

  if (checkbox.checked) {
    checkIcon.style.display = 'block';
    customCheckbox.style.background = '#9c27b0';
    customCheckbox.style.borderColor = '#9c27b0';
    checkIcon.style.color = '#fff';
    if (toggle) {
      toggle.style.background = 'rgba(156,39,176,0.2)';
      toggle.style.borderColor = '#9c27b0';
    }
  } else {
    checkIcon.style.display = 'none';
    customCheckbox.style.background = '#fff';
    customCheckbox.style.borderColor = '#9c27b0';
    if (toggle) {
      toggle.style.background = 'rgba(156,39,176,0.1)';
      toggle.style.borderColor = 'rgba(156,39,176,0.3)';
    }
  }
}
...
function aggCatSel() {
  ...
  if (tipo === 'income') {
    ...
    // Nascondi "Pago al Partner" per entrate
    var pagoPartnerGroup = document.getElementById('pagoPartnerGroup');
    if (pagoPartnerGroup) pagoPartnerGroup.style.display = 'none';
    ...
    // NUOVO - Mostra "Pago al Partner" per uscite (solo modalità coppia)
    var pagoPartnerGroup = document.getElementById('pagoPartnerGroup');
    if (pagoPartnerGroup && DB.config && DB.config.mode === 'couple') {
      pagoPartnerGroup.style.display = 'block';
      // Aggiorna nome partner nel testo
      var nomeSpan = document.getElementById('pagoPartnerNome');
      if (nomeSpan && DB.config.partnerName) {
        nomeSpan.textContent = DB.config.partnerName;
      }
    } else if (pagoPartnerGroup) {
      pagoPartnerGroup.style.display = 'none';
    }
...
function salva(e) {
  ...
  // NUOVO - Gestione "Pago al Partner" (uscita per saldare debito)
  var pagoPartnerCheckbox = document.getElementById('tPagoPartner');
  if (t.tipo === 'expense' && pagoPartnerCheckbox && pagoPartnerCheckbox.checked) {
    t.saldoDebito = true; // Flag: questa NON è una spesa normale, è un trasferimento
    // Aggiungi nota automatica
    var partnerNamePago = DB.config && DB.config.partnerName ? DB.config.partnerName : 'Partner';
    var notaPago = '💸 Pagamento a ' + partnerNamePago + ' (saldo debito)';
    if (t.note) {
      t.note = notaPago + '\n' + t.note;
    } else {
      t.note = notaPago;
...
function mostraTrans() {
  ...
  if (listaFiltrata.length === 0) {
    ...
    listaFiltrata.forEach(function(t) {
      ...
      // Viola = partner ha pagato (condiviso o recupero virtuale) OPPURE pago al partner
      // Rosso = utente ha pagato (condiviso)
      var isPartnerPaid = (t.chiHaPagato === 'partner' || t.tipo === 'partner_payment');
      var isPagoPartner = t.saldoDebito === true; // "Pago al Partner" = sempre viola

      if (isPartnerPaid || isPagoPartner) {
        // Partner ha pagato OPPURE tu paghi il partner - sempre viola
      ...
      if (!t.condiviso && !t.virtualRecovery && !isPartnerPaid && !isPagoPartner) {
...
function resetCampiModal() {
  ...
  // Reset pago al partner
  var tPagoPartner = document.getElementById('tPagoPartner');
  if (tPagoPartner) tPagoPartner.checked = false;
  var pagoPartnerInfo = document.getElementById('pagoPartnerInfo');
  if (pagoPartnerInfo) pagoPartnerInfo.style.display = 'none';
  ...
  if (typeof aggiornaVisualCheckboxPagoPartner === 'function') aggiornaVisualCheckboxPagoPartner();
...
function calcolaSpeseMese(anno, mese) {
  ...
  DB.transazioni.forEach(function(t) {
    ...
    if (t.virtualRecovery) return;
    if (t.saldoDebito) return; // Escludi "Pago al Partner" dalle statistiche spese
...
function aggiornaListaConti() {
  ...
  DB.contiPersonalizzati.forEach(function(conto, index) {
    ...
    html += '<div style="display:flex;align-items:center;gap:8px">';
    html += '<div class="drag-handle" style="color:#a1a1aa;font-size:1em;cursor:grab">⠿</div>';
