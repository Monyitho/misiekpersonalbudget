...
function aggiornaListaObiettiviModal() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    // Card obiettivo compatta
    html += '<div style="background:var(--card);padding:14px;border-radius:12px;margin-bottom:10px;border-left:4px solid ' + progressColor + '">';

    // Header: icona + nome + target su una riga
    html += '<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">';
    html += '<div style="font-size:1.8em">' + obj.icona + '</div>';
    html += '<div style="flex:1;min-width:0">';
    html += '<div style="font-weight:700;font-size:1em;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">' + obj.nome + '</div>';
    html += '<div style="font-size:0.8em;color:#7f8c8d">Target: ' + formatEuro(obj.target) + '</div>';
    html += '</div>';
    html += '</div>';
    ...
    html += '<span style="font-size:0.85em;font-weight:600;color:' + progressColor + '">' + formatEuro(risparmiato) + '</span>';
    html += '<span style="font-size:0.8em;font-weight:700;color:' + progressColor + '">' + percentuale + '%</span>';
    html += '</div>';
    html += '<div style="background:var(--border);height:10px;border-radius:5px;overflow:hidden">';
    html += '<div style="background:' + progressColor + ';height:100%;width:' + Math.min(percentuale, 100) + '%;border-radius:5px"></div>';
    html += '</div>';
    html += '</div>';

    // Date su una riga
    if (obj.dataInizio || obj.dataFine) {
      html += '<div style="display:flex;justify-content:center;gap:15px;margin-bottom:10px;font-size:0.75em;color:#888">';
      if (obj.dataInizio) {
        var dInizio = new Date(obj.dataInizio);
        html += '<span>📅 ' + dInizio.getDate() + '/' + (dInizio.getMonth()+1) + '/' + dInizio.getFullYear() + '</span>';
      }
      if (obj.dataFine) {
        var dFine = new Date(obj.dataFine);
        html += '<span>🏁 ' + dFine.getDate() + '/' + (dFine.getMonth()+1) + '/' + dFine.getFullYear() + '</span>';
      }
      html += '</div>';
    }
    ...
    html += '<button onclick="modificaRisparmiato(' + index + ')" style="flex:1;background:#3498db;color:white;border:none;padding:10px;border-radius:8px;cursor:pointer;font-size:0.85em;font-weight:600">✏️ Modifica</button>';
...
function apriModalModificaConto(id) {
  ...
  // Calcola saldo attuale (saldo iniziale + transazioni)
  var saldoIniziale = (DB.saldiIniziali && DB.saldiIniziali[id]) ? DB.saldiIniziali[id] : 0;
  var saldoTransazioni = 0;
  DB.transazioni.forEach(function(t) {
    if (t.conto === id) {
      if (t.tipo === 'income') {
        saldoTransazioni += t.importo;
      } else if (t.tipo === 'expense') {
        saldoTransazioni -= t.importo;
      }
    }
  });
  var saldoAttuale = saldoIniziale + saldoTransazioni;
  document.getElementById('contoSaldoAttuale').value = saldoAttuale.toFixed(2);
  document.getElementById('contoSaldoAttuale').dataset.saldoTransazioni = saldoTransazioni;
