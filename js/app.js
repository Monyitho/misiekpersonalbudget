...
function selezionaChiHaPagato(who, el) {
  ...
  // Nascondi metodo di pagamento se paga il partner (non riguarda i miei conti)
...
function handleTouchStart(e) {
  ...
  // Usa data-trans-idx per l'indice corretto nel DB
  var idx = swipeState.element.getAttribute('data-trans-idx');
  swipeState.transId = idx !== null ? parseInt(idx) : null;
}
  ...
  if (listaFiltrata.length === 0) {
    ...
    dateOrdinate.forEach(function(dataKey) {
      ...
      headerGiorno.innerHTML = '<div class="day-info"><span class="day-name">' + nomeGiorno + '</span><span class="day-date">' + giorno + ' ' + meseNome + ' ' + annoStr + '</span></div>' +
        ...
        (totaleEntrate > 0 ? '<span class="day-income">+€' + totaleEntrate.toFixed(2) + '</span>' : '') +
        (totaleUscite > 0 ? '<span class="day-expense">-€' + totaleUscite.toFixed(2) + '</span>' : '') +
        '</div>';
      ...
      var amountPrefix = t.tipo === 'income' ? '+' : '-';
      ...
      header.innerHTML =
        ...
        '<div class="trans-content">' +
          '<div class="trans-main-row">' +
            ...
            '<div class="trans-amount">' + amountPrefix + '€' + t.importo.toFixed(2) + '</div>' +
          '</div>' +
...
function modifica(id) {
  ...
  if (document.getElementById('tmetodo')) {
    if (t.pagamentoMisto && t.metodiPagamento && t.metodiPagamento.length > 0) {
      ...
      document.getElementById('tmetodo').value = t.metodo || 'webank';
    }
  }
  ...
  if (pagamentoMistoCheckbox && t.pagamentoMisto && t.metodiPagamento && t.metodiPagamento.length > 0) {
    ...
    if (tmistoValoreBuoni) tmistoValoreBuoni.textContent = '1 buono = €' + valoreUnitario.toFixed(2);
...
function salvaDebitoCondiviso() {
  ...
  if (nuovoSaldoAttuale > 0) {
    mostraToast('✅ ' + partnerName + ' ti deve €' + valore.toFixed(2), 'success');
  } else if (nuovoSaldoAttuale < 0) {
    mostraToast('✅ Devi €' + valore.toFixed(2) + ' a ' + partnerName, 'success');
  } else {
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    // Calcola giorni rimanenti e risparmio mensile necessario
    var giorniRimasti = 0;
    ...
    if (obj.dataFine) {
      ...
      if (mesiRimasti > 0 && mancante > 0) {
        risparmioMensileNecessario = mancante / mesiRimasti;
      }
    }

    // Card obiettivo espansa
    html += '<div style="background:var(--card);border-radius:16px;overflow:hidden;box-shadow:0 4px 15px rgba(0,0,0,0.1);border:1px solid var(--border)">';

    // Header con colore
    html += '<div style="background:' + color + ';padding:14px 16px">';
    html += '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">';
    html += '<div style="display:flex;align-items:center;gap:10px">';
    html += '<span style="font-size:1.6em">' + obj.icona + '</span>';
    html += '<span style="font-weight:800;color:#fff;font-size:1.1em">' + obj.nome + '</span>';
    html += '</div>';
    html += '<span style="background:rgba(255,255,255,0.25);padding:5px 12px;border-radius:20px;font-size:0.9em;font-weight:800;color:#fff">' + percentuale.toFixed(1) + '%</span>';
    html += '</div>';

    // Date nell'header
    if (obj.dataInizio || obj.dataFine) {
      html += '<div style="display:flex;gap:10px;margin-top:8px;font-size:0.78em;color:rgba(255,255,255,0.9)">';
      if (obj.dataInizio) {
        var dI = new Date(obj.dataInizio);
        html += '<span>📅 ' + dI.toLocaleDateString('it-IT', {day:'2-digit',month:'short',year:'numeric'}) + '</span>';
      }
      if (obj.dataInizio && obj.dataFine) {
        html += '<span style="color:rgba(255,255,255,0.6)">→</span>';
      }
      if (obj.dataFine) {
        var dF = new Date(obj.dataFine);
        html += '<span>🏁 ' + dF.toLocaleDateString('it-IT', {day:'2-digit',month:'short',year:'numeric'}) + '</span>';
      }
      html += '</div>';
    }
    html += '</div>';
    ...
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
    html += '<span style="font-size:0.82em;font-weight:600;color:var(--text)">Progresso</span>';
    html += '<span style="font-size:0.85em;font-weight:800;color:' + progressColor + '">' + percentuale.toFixed(1) + '%</span>';
    html += '</div>';
    html += '<div style="background:var(--border);height:12px;border-radius:6px;overflow:hidden">';
    html += '<div style="background:' + progressColor + ';height:100%;width:' + Math.min(percentuale, 100) + '%;transition:width 0.4s;border-radius:6px"></div>';
    html += '</div>';
    html += '</div>';

    // Griglia stats 3 colonne
    html += '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px">';

    html += '<div style="padding:12px 6px;background:rgba(76,175,80,0.1);border-radius:10px;text-align:center;border:1px solid rgba(76,175,80,0.2)">';
    html += '<div style="font-size:0.68em;color:#2e7d32;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">💵 Risparmiato</div>';
    html += '<div style="font-size:1em;font-weight:800;color:#2e7d32">' + formatEuro(risparmiato) + '</div>';
    html += '</div>';

    html += '<div style="padding:12px 6px;background:rgba(33,150,243,0.1);border-radius:10px;text-align:center;border:1px solid rgba(33,150,243,0.2)">';
    html += '<div style="font-size:0.68em;color:#1976d2;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">🎯 Obiettivo</div>';
    html += '<div style="font-size:1em;font-weight:800;color:#1976d2">' + formatEuro(obj.target) + '</div>';
    html += '</div>';

    html += '<div style="padding:12px 6px;background:rgba(255,152,0,0.1);border-radius:10px;text-align:center;border:1px solid rgba(255,152,0,0.2)">';
    html += '<div style="font-size:0.68em;color:#e65100;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px">💰 Mancante</div>';
    html += '<div style="font-size:1em;font-weight:800;color:#e65100">' + formatEuro(mancante) + '</div>';
    html += '</div>';

    html += '</div>';

    // Info aggiuntive (giorni rimasti + risparmio mensile necessario)
    if (obj.dataFine && !isScaduto && mancante > 0) {
      html += '<div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px">';

      html += '<div style="padding:10px;background:rgba(156,39,176,0.1);border-radius:10px;text-align:center;border:1px solid rgba(156,39,176,0.2)">';
      html += '<div style="font-size:0.68em;color:#7b1fa2;font-weight:700;margin-bottom:4px">⏱️ Giorni Rimasti</div>';
      html += '<div style="font-size:1em;font-weight:800;color:#7b1fa2">' + giorniRimasti + '</div>';
      html += '</div>';

      if (risparmioMensileNecessario > 0) {
        html += '<div style="padding:10px;background:rgba(0,188,212,0.1);border-radius:10px;text-align:center;border:1px solid rgba(0,188,212,0.2)">';
        html += '<div style="font-size:0.68em;color:#00838f;font-weight:700;margin-bottom:4px">📆 Al Mese Necessario</div>';
        html += '<div style="font-size:1em;font-weight:800;color:#00838f">' + formatEuro(risparmioMensileNecessario) + '</div>';
        html += '</div>';
      }
    ...
    if (isScaduto) {
      html += '<div style="padding:8px;background:rgba(149,165,166,0.15);border-radius:8px;text-align:center;color:#7f8c8d;font-size:0.82em;font-weight:600;margin-bottom:12px">⏰ Scaduto</div>';
    }

    if (percentuale >= 100) {
      html += '<div style="text-align:center;padding:10px;background:rgba(76,175,80,0.15);border-radius:10px;color:#2e7d32;font-weight:700;font-size:0.9em;margin-bottom:12px">🎉 OBIETTIVO RAGGIUNTO!</div>';
    }

    // Bottoni azione
    html += '<div style="display:flex;gap:8px">';
    html += '<button onclick="modificaObiettivoCompleto(' + index + ')" style="flex:1;background:linear-gradient(135deg,#3498db,#2980b9);color:#fff;border:none;padding:11px;border-radius:10px;cursor:pointer;font-size:0.85em;font-weight:700">✏️ Modifica</button>';
    html += '<button onclick="eliminaObiettivoInvestimento(' + index + ')" style="background:linear-gradient(135deg,#e74c3c,#c0392b);color:#fff;border:none;padding:11px 14px;border-radius:10px;cursor:pointer;font-size:0.85em;font-weight:700">🗑️</button>';
    html += '</div>';
...
function modificaObiettivoCompleto(index) {
  ...
  modal.innerHTML =
    '<div class="modal-header" style="background:linear-gradient(135deg,#ff9800,#f57c00);padding:18px 20px;border-radius:16px 16px 0 0;display:flex;align-items:center;justify-content:space-between">' +
      '<h3 style="margin:0;color:#fff;font-size:1.1em">✏️ Modifica Obiettivo</h3>' +
      '<button onclick="chiudiModal()" style="background:rgba(255,255,255,0.2);color:#fff;border:none;width:36px;height:36px;border-radius:8px;cursor:pointer;font-size:1.2em;font-weight:700">✕</button>' +
    '</div>' +
    '<div style="padding:20px;display:flex;flex-direction:column;gap:14px">' +
      '<div class="form-group">' +
        '<label>Nome Obiettivo</label>' +
        '<input type="text" id="editObNome" value="' + obj.nome + '" placeholder="es: Acconto Casa...">' +
      '</div>' +
      '<div class="form-group">' +
        '<label>Icona</label>' +
        '<select id="editObIcona" style="padding:12px;border:2px solid var(--border);border-radius:10px;font-size:1.4em;background:var(--bg);color:var(--text);width:100%">' + iconeHtml + '</select>' +
      '</div>' +
      '<div class="form-group">' +
        '<label>Target (€)</label>' +
...
function calcolaDifferenzaMista() {
  ...
  if (pagataDifferenzaDa === 'partner') {
    ...
    var partnerName = DB.config && DB.config.partnerName ? DB.config.partnerName : 'Partner';
    if (partnerText) {
      partnerText.innerHTML = '👤 ' + partnerName + ' paga la differenza<br><span style="font-size:0.85em;color:#9c27b0;font-weight:800">€' + differenza.toFixed(2) + '</span>';
    }
  }
